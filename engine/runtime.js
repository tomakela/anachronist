import { integer, tuple, list } from "./ini.js";
import { BackgroundTasks, instantiate, textDuration } from "./script.js";
...
export class Runtime extends DeterministicVM {
  constructor(project, { host, storage, clock = { now: () => globalThis.performance.now() }, scheduler = { requestFrame: (callback) => globalThis.requestAnimationFrame(callback), setTimeout: (callback, delay) => globalThis.setTimeout(callback, delay), clearTimeout: (id) => globalThis.clearTimeout(id) }, initialRoom, debugMode = false } = {}) {
    if (!host) throw new TypeError("Runtime requires an explicit host container");
    const { configuration: game, ui, rooms, items, graphics, animations, bitmaps, handlers, input } = project;
    super();
    Object.assign(this, { game, ui, rooms, items, graphics, animations, bitmaps, handlers, input, debugMode, host, clock, scheduler, initialRoom });
    this.document = host.ownerDocument; this.window = this.document.defaultView;
    ...
    if (!storage) throw new TypeError("Runtime requires an explicit storage adapter");
    this.storage = storage.exists ? storage : new SaveStorage(storage, this.saveIdentity.packageId);
    ...
    this.canvas = this.document.createElement("canvas"); this.canvas.width = this.width; this.canvas.height = this.height;
    ...
    this.coarsePointer = this.window.matchMedia("(pointer: coarse)").matches;
    this.cursorMode = storage?.getItem("anachronist.cursor-mode") || "drag";
    ...
    this.host.replaceChildren(this.canvas, this.settings()); this.host.ariaBusy = "false";
  }
  start() {
    this.dispatch("game.start", []);
    if (this.initialRoom) {
      const selection = typeof this.initialRoom === "string" ? { room: this.initialRoom } : this.initialRoom;
      this.queue.unshift({ op: "enter", ...selection });
    }
    this.host.addEventListener("pointerdown", (event) => this.pointerDown(event));
    this.host.addEventListener("pointermove", (event) => this.pointerMove(event));
    ...
    this.canvas.focus(); this.last = this.clock.now(); this.scheduler.requestFrame((now) => this.frame(now));
  }
  keyboard(event) {
  ...
  settings() {
    ...
    debug.addEventListener("change", () => { this.window.location.href = debugUrl(this.window.location.href, debug.checked); });
    ...
    for (const [value, label] of [["direct", "Point where I touch"], ["drag", "Drag cursor"]]) {
      const row = this.document.createElement("label"), radio = this.document.createElement("input"); radio.type = "radio"; radio.name = "cursor-mode"; radio.value = value; radio.checked = this.cursorMode === value;
      radio.addEventListener("change", () => { this.cursorMode = value; storage?.setItem("anachronist.cursor-mode", value); });
      row.append(radio, ` ${label}`); choices.append(row);
    }
    const fullscreen = this.document.createElement("button"); fullscreen.type = "button"; fullscreen.className = "fullscreen-toggle"; fullscreen.textContent = "Enter full screen";
    fullscreen.hidden = !this.document.fullscreenEnabled; fullscreen.addEventListener("click", () => this.toggleFullscreen());
    this.document.addEventListener("fullscreenchange", () => { fullscreen.textContent = this.document.fullscreenElement ? "Exit full screen" : "Enter full screen"; });
    choices.append(fullscreen);
    for (const [label, action] of [["Save", () => this.saveGame()], ["Load", () => this.loadGame()], ["Restart", () => this.restartGame()]]) {
      const control = this.document.createElement("button"); control.type = "button"; control.className = "menu-action"; control.textContent = label; control.addEventListener("click", action); choices.append(control);
    }
  ...
  reportSaveError(action, error) { this.window.alert(`${action} failed: ${error.message}`); console.error(error); }
  saveGame() {
    try {
      if (this.storage.exists() && !this.window.confirm("Overwrite your existing saved progress?")) return;
      this.storage.write(snapshotRuntime(this, this.saveIdentity));
      this.window.alert("Game saved.");
  ...
  restartGame() { if (this.window.confirm("Restart and discard all current progress?")) this.window.location.reload(); }
  async toggleFullscreen() {
    if (this.document.fullscreenElement) {
      await this.document.exitFullscreen(); this.window.screen.orientation?.unlock?.(); return;
    }
    await this.host.requestFullscreen({ navigationUI: "hide" });
    await this.window.screen.orientation?.lock?.("landscape").catch(() => {});
  }
  ...
  pointerUp(event) {
    ...
    if (!this.touch.long && !this.touch.moved) {
      const nowValue = this.clock?.now?.() ?? globalThis.performance.now();
      const button = nowValue - this.touch.startedAt >= this.longTouchMilliseconds ? 2 : 0;
      this.touch.long = button === 2;
      const now = nowValue, fast = button === 0 && this.lastTap && now - this.lastTap.time <= this.doubleTouchMilliseconds && !touchMoved(this.lastTap.point, this.touchCursor, this.doubleTouchMoveTolerance);
  ...
  cancelTouch(event) { if (this.touch?.id === event.pointerId) { (this.scheduler?.clearTimeout || globalThis.clearTimeout)(this.touch.timer); this.touch = null; } }
