...
export default function App() {
  ...
  const upload = useRef<HTMLInputElement>(null), [filter, setFilter] = useState(""), [collapsed, setCollapsed] = useState<Set<string>>(new Set());
  ...
  const roomFiles = service.entries.filter(entry => entry.kind === "file" && /(^|\/)rooms\/[^/]+\/room\.ini$/.test(entry.path));
  useEffect(() => {
  ...
  const closeFile = (path: string) => { const index = documents.findIndex(item => item.path === path); if (!service.closeFile(path)) return; if (active === path) { const next = documents[index + 1] || documents[index - 1]; setActive(next?.path); if (next) setView(/rooms\/[^/]+\/room\.ini$/.test(next.path) ? "room" : "source"); } };
