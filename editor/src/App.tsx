...
export default function App() {
  const [, render] = useState(0), [active, setActive] = useState<string>(), [output, setOutput] = useState("Ready."), [view, setView] = useState<"source" | "form">("source"), [diagnostics, setDiagnosticsState] = useState<any[]>([]), [index, setIndex] = useState(emptyIndex);
  ...
  const openFile = async (path: string) => { try { await service.readFile(path); setActive(path); setView("source"); } catch (error) { setOutput(String(error)); } };
