...
import { DevelopmentGameProjectAdapter, TauriProjectAdapter } from "./fileSystemAdapter";
...
export default function App() {
  ...
  useEffect(() => {
    if (TauriProjectAdapter.available()) TauriProjectAdapter.open().then(adapter => adapter && service.openPackageDirectory(adapter)).catch(error => setOutput(String(error)));
    else service.openPackageDirectory(new DevelopmentGameProjectAdapter()).catch(error => setOutput(String(error)));
  }, []);
  const openNativeProject = async () => { try { const adapter = await TauriProjectAdapter.open(); if (adapter) { await service.openPackageDirectory(adapter); setActive(undefined); setOutput(`Opened ${adapter.name}.`); } } catch (error) { setOutput(String(error)); } };
  ...
  return <main className="app">
    <header><strong><span className="mark">A</span> Anachronist Editor</strong><span className="project">{service.project?.name || "No project open"}{service.hasDirtyDocuments && " • Unsaved"}</span>{TauriProjectAdapter.available() && <button onClick={openNativeProject}>Open project</button>}<button disabled={!service.project} onClick={() => setView("playtest")}>Playtest</button><button disabled={!service.project} onClick={() => service.saveAllFiles().catch(error => setOutput(String(error)))}>Save all</button><button disabled={!service.project} onClick={() => { if (service.closeProject()) setActive(undefined); }}>Close</button></header>
